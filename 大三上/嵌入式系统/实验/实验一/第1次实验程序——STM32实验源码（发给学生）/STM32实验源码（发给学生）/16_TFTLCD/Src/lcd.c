...
{ 			
	...
	delay_ms(50); // delay 50 ms 
  lcddev.id =LCD_ReadReg(0x0000);   //LCD_ReadReg(0x0000)
	printf(" DAT ID:%x\r\n",lcddev.id); //打印LCD ID   
   	if(lcddev.id<0XFF||lcddev.id==0XFFFF)//读到ID不正确,新增lcddev.id==0X9300判断，因为9341在未被复位的情况下会被读成9300
	{	
					...
					if(lcddev.id==0x8000)lcddev.id=0x5510;//NT3 5510读回的ID是8000H,为方便区分,我们强制设置为5510
	} 
