...
static uint16_t POINT_COLOR = DARKBLUE; //画笔颜色
...
/**
 * @description: 写寄存器
 * @param {uint16_t} LCD_Reg 寄存器地址
 * @param {uint16_t} LCD_RegValue 要写入的数据
 * @return {*}
 */
...
/**
 * @description: LCD写GRAM
 * @param {uint16_t} RGB_Code颜色值
 * @return {*}
 */
...
/**
 * @description: 通过该函数转换 从ILI93xx读出的数据为GBR格式，而我们写入的时候为RGB格式。
 * @param {uint16_t} c GBR格式的颜色值
 * @return {*} 返回值：RGB格式的颜色值
 */
...
uint16_t LCD_ReadPoint(uint16_t x, uint16_t y)
{
    ...
    if (g_lcddev.id == 0X5510) //9341/NT35310/NT35510要分2次读出
    {
        opt_delay(2);
        b = LCD_RD_DATA();
...
/**
 * @description: 设置光标位置
 * @param {uint16_t} Xpos 横坐标
 * @param {uint16_t} Ypos 纵坐标
 * @return {*}
 */
...
/**
 * @description: 设置LCD的自动扫描方向(一般设置为L2R_U2D即可,如果设置为其他扫描方式,可能导致显示不正常.)
 * @param {uint8_t} dir 0~7,代表8个方向(具体定义见lcd.h)
 * @return {*}
 */
void LCD_Scan_Dir(uint8_t dir)
{
    ...
    if (g_lcddev.id == 0X5510)
    {
        ...
        if (regval & 0X20)
        {
            if (g_lcddev.width < g_lcddev.height) //交换X,Y
            {
                temp = g_lcddev.width;
...
/**
 * @description: SSD1963 背光设置
 * @param {uint8_t} pwm 背光等级,0~100.越大越亮
 * @return {*}
 */
 ...
 * @param {uint16_t} sx sx,sy:窗口起始坐标(左上角)
 * @param {uint16_t} sy
 * @param {uint16_t} width width,height:窗口宽度和高度,必须大于0
...
void LCD_Clear(uint16_t color)
{
    ...
    totalpoint *= g_lcddev.height; //得到总点数
 ...
 * @param {uint16_t} sx (sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)
 * @param {uint16_t} sy
 * @param {uint16_t} ex
 * @param {uint16_t} ey
 * @param {uint16_t} color 要填充的颜色
 * @return {*}
 */
...
/**
 * @description: 画线
 * @param {uint16_t} x1 x1,y1:起点坐标
 * @param {uint16_t} y1
 * @param {uint16_t} x2 x2,y2:终点坐标
 * @param {uint16_t} y2
 * @return {*}
 */
...
/**
 * @description: 画矩形
 * @param {uint16_t} x1 (x1,y1),(x2,y2):矩形的对角坐标
 * @param {uint16_t} y1
 * @param {uint16_t} x2
 * @param {uint16_t} y2
 * @return {*}
 ...
 * @param {uint16_t} x 起始坐标
 * @param {uint16_t} y
 * @param {uint8_t} num 要显示的字符
 * @param {uint8_t} size 字体大小 12/16/24
 * @param {uint8_t} mode 叠加方式(1)还是非叠加方式(0)
 * @return {*}
 */
 ...
 * @param {uint16_t} x 起点坐标
 * @param {uint16_t} y
 * @param {uint32_t} num 数字的位数
 * @param {uint8_t} len  字体大小
 * @param {uint8_t} size 颜色
 * @return {*}
 */
...
/**
 * @description: 显示数字,高位为0
 * @param {uint16_t} x 起点坐标
 * @param {uint16_t} y
 * @param {uint32_t} num 数值
 * @param {uint8_t} len 长度
 * @param {uint8_t} size 字体大小
 * @param {uint8_t} mode [7]:0,不填充;1,填充0; [6:1]:保留; [0]:0,非叠加显示;1,叠加显示
 * @return {*}
 */
...
/**
 * @description: 在指定座标范围显示一副图片,图片取模格式为水平扫描，16位颜色模式, 需要将图片数据转换为数组
 * @param {uint16_t} StartX 行起始座标
 * @param {uint16_t} StartY 列起始座标
 * @param {uint16_t} Xend   行结束座标
 * @param {uint16_t} Yend   列结束座标
 * @param {uint8_t} *pic    图片头指针
 * @return {*}
 */
...
void LCD_Init(void)
{
    ...
    g_lcddev.id = 0x5510;
    CONSOLE_INFO("LCD ID:%x", g_lcddev.id); //打印LCD ID
    ...
    LCD_Display_Dir(1); //dir:0,竖屏；1,横屏
